...
/**
 * @brief Tool to generate random mpv games for testing solvers
 *
 * This tool generates random mpv games in DOT format and saves them
 * to a specified directory.
 */
class MPGameGenerator {
  public:
    /**
     * @brief Main function for the mpv game generator tool
     */
    static int run(int argc, char *argv[]) {
        try {
            po::options_description desc("Mean Payoff Game Generator Options");
            ...
            desc.add_options()("max-weight", po::value<int>()->default_value(5), "Maximum weight for mean payoff games");
            ...
            return generate_games(output_dir, count, vertices, max_weight, min_out_degree, max_out_degree, seed, verbose);
  ...
  private:
    /**
     * @brief Generate random mpv games
     */
    static int generate_games(const std::string &output_dir,
        ...
        if (verbose) {
            std::cout << "Generating " << count << " mean payoff games with "
        ...
        for (int i = 0; i < count; ++i) {
            std::string filename = generate_game_file(output_dir, i, vertices, max_weight, min_out_degree, max_out_degree, gen);
        ...
        if (verbose) {
            std::cout << std::endl
                      << "Successfully generated " << count << " mean-payoff games" << std::endl;
        }
    ...
    static std::string generate_game_file(const std::string &output_dir,
        ...
        const auto filename = "mp_game_" + std::to_string(game_index + 1) + ".dot";
        ...
        generate_mp_game(file, vertices, max_weight, min_out_degree, max_out_degree, gen);
        file.close();
        return filename;
    }

    /**
     * @brief Generate a random mean payoff game
     */
    static void generate_mp_game(std::ofstream &file,
                                 int vertices,
                                 int max_weight,
                                 int min_out_degree,
                                 int max_out_degree,
...
namespace ggg_tools {
int run_generate_mpv_games(int argc, char *argv[]) {
    return MPGameGenerator::run(argc, argv);
}
} // namespace ggg_tools
