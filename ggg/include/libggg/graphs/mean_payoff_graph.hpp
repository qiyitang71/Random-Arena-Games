...
DEFINE_GAME_GRAPH(MeanPayoff, MEAN_PAYOFF_VERTEX_FIELDS, MEAN_PAYOFF_EDGE_FIELDS, MEAN_PAYOFF_GRAPH_FIELDS)

#undef MEAN_PAYOFF_VERTEX_FIELDS
#undef MEAN_PAYOFF_EDGE_FIELDS
#undef MEAN_PAYOFF_GRAPH_FIELDS

// Additional utility functions specific to mean payoff games

/**
 * @brief Checks if the given MeanPayoffGraph is valid.
 *
 * A valid MeanPayoffGraph must satisfy the following conditions for every vertex:
 *   - The 'player' property must be either 0 or 1.
 *   - The vertex must have at least one outgoing edge.
 *
 * @param graph The MeanPayoffGraph to validate.
 * @return true if the graph is valid, false otherwise.
 */
inline bool is_valid(const MeanPayoffGraph &graph) {
...
/**
 * @brief Check for duplicate edges in the mean payoff graph.
 *
 ...
 *
 * @param graph The MeanPayoffGraph to check for duplicate edges.
 * @throws std::runtime_error if duplicate edges are found.
 */
inline void check_no_duplicate_edges(const MeanPayoffGraph &graph) {
    std::set<std::pair<MeanPayoffVertex, MeanPayoffVertex>> seen_edges;
